...
- [x] **DB-1** — Add `topup_request_status` enum to `src/lib/db/schema/enums.ts`
  - Add `pgEnum("topup_request_status", ["pending","completed","failed","expired","cancelled"])`
  - **Satisfies**: Req 10.3

- [x] **DB-2** — Create `topup_request` table in `src/lib/db/schema/wallet-tables.ts`
...
- [x] **DB-3** — Add `topupRequestId` FK column to `wallet_transaction` in `src/lib/db/schema/wallet-tables.ts`
  - Nullable `text` column referencing `topup_request.id`, no cascade
  - Export `topupRequests` from `src/lib/db/schema/index.ts`
  - **Satisfies**: Req 10.4

- [x] **DB-4** — Generate and apply Drizzle migration
...
- [x] **DB-5** — Add `getOrderTotalQuantity` helper to `src/lib/db/queries/order-queries.ts`
...
- [x] **DB-6** — Create `src/lib/db/queries/topup-queries.ts`
...
- [x] **BE-1** — Create `src/lib/validators/checkout-schemas.ts`
  - `checkoutItemSchema`, `checkoutCartSchema`, `CheckoutCartInput` type
  - No price field (server resolves pricing)
  - **Satisfies**: Req 2.2

- [x] **BE-2** — Create `src/lib/validators/topup-schemas.ts`
...
- [x] **BE-3** — Create `src/lib/actions/checkout-actions.ts` with `checkoutCart()`
...
- [x] **BE-4** — Create `src/lib/actions/topup-actions.ts` with `createTopupRequest()`, `cancelTopupRequest()`, `getTopupRequest()`
...
- [x] **BE-5** — Modify `src/lib/actions/delivery-actions.ts`
...
- [x] **INT-1** — Create `src/lib/integrations/wise/types.ts`
  - `WiseCreatePaymentInput`, `WiseCreatePaymentResult`, `WiseWebhookEvent` union type
  - `WiseGatewayError` class
  - **Satisfies**: Req 4.4

- [x] **INT-2** — Create `src/lib/integrations/wise/client.ts`
  ...
  - Mock implementation for dev/test (auto-enabled when `WISE_API_TOKEN` not set)
  - `getWiseClient()` factory function
  - **Satisfies**: Req 4.4, 4.5

- [x] **INT-3** — Create `src/lib/integrations/wise/signature.ts`
...
- [x] **INT-4** — Create `src/app/api/webhooks/wise/route.ts`
...
- [x] **INT-5** — Create `src/app/api/cron/expire-topups/route.ts`
...
- [x] **INT-6** — Add env vars to `.env.example`
...
- [x] **FE-1** — Update `src/lib/cart-context.tsx`
...
- [x] **FE-2** — Create `src/lib/hooks/use-topup-status-polling.ts`
...
- [x] **FE-3** — Create `src/components/portal/cart-checkout-button.tsx`
...
- [x] **FE-4** — Create `src/app/portal/wallet/insufficient-balance-banner.tsx`
...
- [x] **FE-5** — Create `src/app/portal/wallet/topup-dialog.tsx`
  ...
  - Render QR code (qrcode.react), countdown to `expiresAt`
  - Subscribe to `useTopupStatusPolling`, react to status changes
  - "Cancel" button calls `cancelTopupRequest()`
  - "I've paid" manual refresh button
  - **Satisfies**: Req 4.7, 6.1–6.2

- [x] **FE-6** — Create `src/app/portal/wallet/checkout-resume-modal.tsx`
...
- [x] **FE-7** — Update `src/app/portal/wallet/page.tsx` + create `wallet-page-client.tsx`
  - Server component passes `returnToCheckout` and `balance` to client
  - Client renders `InsufficientBalanceBanner` when appropriate
  - "Top up" button always visible, opens `TopupDialog`
  - Wire `useTopupStatusPolling` to open `CheckoutResumeModal` on completion when `pendingCheckout=true`
  - **Satisfies**: Req 3.3, 6.3

- [x] **FE-8** — Create `src/app/portal/checkout/page.tsx`
...
- [x] **FE-9** — Wire `CartCheckoutButton` into the portal cart sheet
  - Add `showCheckout` prop to `shopping-cart-02.tsx` and `cart-button-wrapper.tsx`
  - Pass `showCheckout` from portal's `site-header.tsx`
  - **Satisfies**: Req 1.1

- [x] **FE-10** — Bind `ownerCustomerId` in portal layout
  - In `src/app/portal/portal-shell.tsx`, call `cart.bindOwner(userId)` in `useEffect`
...
- [x] All DB tasks done, migration applied, no TypeScript errors
- [x] All BE tasks done, `pnpm build` passes
- [x] All INT tasks done, webhook route returns correct status codes
- [x] All FE tasks done, portal checkout flow works end-to-end
- [x] `.env.example` updated with all new vars
- [x] `tasks.md` updated with all tasks marked done
