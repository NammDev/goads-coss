    ...
    <>
      ...
      <div className="rounded-[16px] border border-[var(--solid-50)] bg-[var(--solid-25)] p-5 max-md:p-4">
        <div className="flex flex-wrap items-baseline gap-x-3 gap-y-2">
          <span className={cn(siteText.overline, "text-[var(--solid-400)]")}>Columns</span>
          {COLUMNS.map((col, i) => (
            <Fragment key={col}>
              {i > 0 && (
                <span className={cn(siteText.bodyS, "text-[var(--solid-300)]")}>·</span>
              )}
              <span className={cn(siteText.labelS, "text-[var(--solid-700)]")}>
                <span className="text-[var(--solid-400)]">{i + 1}.</span> {col}
              </span>
            </Fragment>
          ))}
        </div>
          ...
          <div className="flex flex-col gap-3">
            {rows.map((row, i) => (
              <div
                key={i}
                className={cn(
                  "rounded-[16px] border border-[var(--solid-50)] bg-[var(--solid-25)] p-5 max-md:p-4",
                    ...
                    return (
                      <Fragment key={label}>
                        <span
                          className={cn(
                            siteText.bodyS,
                            "text-[var(--solid-400)] max-md:col-span-2",
                          )}
                        >
                          {label}
                        </span>
                        <span
                          className={cn(
                            ...
                            value ? "text-[var(--solid-700)]" : "text-[var(--solid-300)]",
                          )}
                        >
                          {value || "—"}
                        </span>
                        {value ? (
                          <button
                            ...
                            className={cn(
                              ...
                              isCopied
                                ? "text-[var(--solid-900)]"
                                : "text-[var(--solid-400)] hover:bg-white hover:text-[var(--solid-900)]",
                            )}
                          >
...
function TextField({
  ...
  return (
    <div
      className={cn(
        "flex min-w-0 flex-col rounded-[16px] border border-[var(--solid-50)] bg-white",
        "transition-colors duration-[500ms] ease-[cubic-bezier(0.19,1,0.22,1)]",
        !readOnly && "focus-within:border-[var(--solid-400)]",
      )}
    >
      <div className="flex items-center justify-between gap-3 border-b border-[var(--solid-50)] px-4 py-3">
        <span className={cn(siteText.overline, "text-[var(--solid-400)]")}>{label}</span>
        {action}
      </div>
      <textarea
        value={value}
