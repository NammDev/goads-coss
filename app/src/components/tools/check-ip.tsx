  ...
  return (
    <>
      ...
      <form
        onSubmit={onSubmit}
        className={cn(
          "flex items-center gap-2 rounded-[16px] border border-[var(--solid-50)] bg-white p-2",
      ...
      >
        <input
          ...
          className={cn(
            siteText.bodyM,
            "flex-1 min-w-0 bg-transparent px-3 py-2 font-mono text-[var(--solid-700)] outline-none",
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 flex-col">
          <p className={cn(siteText.bodyM, "text-[var(--solid-500)]")}>
            {parsedFrom
          ...
          {parsedFrom && (
            <p className={cn(siteText.bodyS, "truncate text-[var(--solid-400)]")}>
      ...
      <div
        className={cn(
          "rounded-[16px] border border-[var(--solid-50)] bg-white p-5 max-md:p-4",
          "transition-colors duration-[500ms] ease-[cubic-bezier(0.19,1,0.22,1)]",
        )}
      >
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col gap-1">
            <span className={cn(siteText.overline, "text-[var(--solid-400)]")}>IP Address</span>
            {info && <StatusBadge valid={info.valid} />}
          </div>

          {loading && !info ? (
            <span className="h-9 w-48 animate-pulse rounded-[6px] bg-[var(--solid-25)] max-md:w-32" />
          ) : info ? (
            <div className="flex items-center gap-3">
              <span
                className={cn(
                  ...
                  "max-md:text-[1.5rem]",
                  info.valid ? "text-[var(--solid-900)]" : "text-[var(--solid-300)]",
                )}
              >
                {info.ip || "—"}
              </span>
              {info.valid && (
                <button
                  ...
                  aria-label="Copy IP"
                  className={cn(
                    "flex size-8 items-center justify-center rounded-[10px] text-[var(--solid-400)] transition-colors duration-200",
                    "hover:bg-[var(--solid-25)] hover:text-[var(--solid-900)]",
                    copied && "text-[var(--solid-900)]",
                  )}
                >
            ...
            <span className={cn(siteText.bodyS, "text-[var(--solid-400)]")}>—</span>
          )}
        </div>
      </div>
      ...
      {((loading && !info) || (info && info.valid)) && (
        <div
          className={cn(
            "overflow-hidden rounded-[16px] border border-[var(--solid-50)] bg-white",
        ...
        >
          {loading && !info
            ? Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between border-b border-[var(--solid-50)] px-5 py-4 last:border-b-0 max-md:px-4"
                >
                  <span className="h-3 w-24 animate-pulse rounded-[6px] bg-[var(--solid-25)]" />
                  <span className="h-3 w-32 animate-pulse rounded-[6px] bg-[var(--solid-25)]" />
                </div>
              ))
            : details.map((row) => (
                <div
                  key={row.label}
                  className="flex items-center justify-between gap-4 border-b border-[var(--solid-50)] px-6 py-4 last:border-b-0 max-md:px-4"
                >
                  <span className={cn(siteText.bodyS, "text-[var(--solid-400)]")}>{row.label}</span>
                  <span className={cn(siteText.labelM, "truncate text-right text-[var(--solid-700)]")}>
                    {row.value || "—"}
                  </span>
                </div>
              ))}
      ...
      {info && !info.valid && !loading && (
        <p className={cn(siteText.bodyS, "text-[var(--solid-400)]")}>
  ...
  return (
    <span className="flex items-center gap-2">
      <span
        className={cn(
          "size-2 rounded-full",
          valid ? "bg-emerald-500" : "bg-[var(--solid-300)]",
        )}
      />
      <span
        className={cn(
          siteText.labelS,
          "tracking-[0.04em]",
          valid ? "text-[var(--solid-900)]" : "text-[var(--solid-400)]",
        )}
      >
        {valid ? "VALID" : "INVALID"}
      </span>
