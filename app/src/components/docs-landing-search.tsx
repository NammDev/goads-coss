// Foreplay docs landing search — pixel-perfect clone with animated shine border.
//
// Structure: <form> > .grid.rounded-[12px] > [shine pseudo-layer] + [content layer]
//   - Shine layer: `before:` with radial-gradient cycling across 300% canvas via
//     shine-pulse animation, mask-composite XOR carves it into a 1px border only.
//   - Content layer: textarea + magnifier icon, surface bg-foreground/[4.5%].
// Textarea is read-only and click dispatches Cmd+K to open the global CommandMenu.

"use client"

export function DocsLandingSearch() {
  const dispatchCmdK = () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content layer — relative border box wraps textarea + magnifier icon.
            Spec: bg foreground/[4.5%], border foreground/[10%], !rounded-lg. */}
