...
interface ForeplayDatePillProps {
...
export function ForeplayDatePill({
  dow,
  dom,
  isToday = false,
  href,
  onClick,
  className,
}: ForeplayDatePillProps) {
  const sharedClass = cn(
    "flex flex-1 cursor-pointer flex-col items-center gap-0 rounded-lg p-3 text-center no-underline",
    "border-[1.5px] bg-white text-[var(--fp-solid-900)]",
    "transition-all duration-200",
    "hover:border-[var(--fp-solid-900)] hover:bg-[var(--fp-solid-25)]",
    isToday ? "border-[var(--fp-solid-900)]" : "border-[var(--fp-solid-200)]",
    className,
  )
