...
// NOTE: Uses --solid-* tokens (light theme)

import { cn } from "@/lib/utils"
import { siteText } from "@/components/atoms/typography"
import { ComparisonTooltipBadge } from "@/components/atoms/comparison-tooltip-badge"
...
export function PricingComparison({
  ...
  return (
    ...
    <div className={cn(
            ...
            <div className="flex flex-col items-center gap-2">
              {title && (
                <div className="text-[var(--solid-900)]">
                  <h2 className={siteText.displayH3}>{title}</h2>
                </div>
              )}
              {description && (
                <div className="text-[var(--solid-500)] [text-wrap:pretty]">
                  <p className={siteText.bodyM}>{description}</p>
                </div>
              )}
            </div>
          </div>
      ...
      {/* .comparison-tooltip */}
      {!hideTooltipBadge && <ComparisonTooltipBadge />}
