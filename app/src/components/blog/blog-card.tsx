...
export function BlogCard({ post, basePath = "/blog", className }: BlogCardProps) {
  return (
    <Link
      ...
      className={cn(
        // .blog-list-card
        "group block overflow-hidden rounded-[20px] bg-background shadow-[0_0_0_1px_var(--ring-color,#ffffff1a)]",
      ...
      <div className="flex flex-1 flex-col gap-4 px-6 pb-6 pt-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 text-foreground">
            <div className={siteText.labelM}>{post.author.name}</div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="text-foreground">
            <div className={cn(siteText.labelL, "line-clamp-2")}>
              {post.title}
            </div>
          </div>
          {/* .line-clamp-2 > .text-alpha-100 > .text-body-m: excerpt */}
          <div className="line-clamp-2">
            <div className="flex-1 text-[var(--alpha-100,#ffffffad)]">
              <div className={siteText.bodyM}>{post.description}</div>
            </div>
          </div>
        </div>
      </div>
