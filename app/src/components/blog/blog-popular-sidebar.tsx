  ...
  return (
    ...
    <div className={cn("flex flex-col items-start gap-5", className)}>
      ...
      <div className="text-[var(--alpha-100,#ffffffad)]">
        <h2 className={siteText.overline}>Popular Blogs</h2>
      </div>
          ...
          return (
            ...
            <div key={post.slug} className="w-full flex-1 text-foreground">
              ...
              <div className="flex flex-col items-stretch gap-[5px] text-left">
                ...
                >
                  {/* h3.text-label-m */}
                  <h3 className={siteText.labelM}>{post.title}</h3>
                  {/* .blog-feed-subttext (no own CSS — pass-through wrapper) */}
                  <div>
                    {/* .text-alpha-100 */}
                    <div className="text-[var(--alpha-100,#ffffffad)]">
                      {/* .text-body-s.line-clamp-2 */}
                      <div className={cn(siteText.bodyS, "line-clamp-2")}>
                        {post.description}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex items-center pt-3">
                ...
                <span className="text-foreground">
                  <span className={siteText.bodyS}>{post.author.name}</span>
                </span>
                ...
                <span className="flex items-center gap-1 text-[var(--alpha-100,#ffffffad)]">
                  {num && <span className={siteText.bodyS}>{num}</span>}
                  <span className={siteText.bodyS}>{label}</span>
                </span>
              </div>
            </div>
          )
