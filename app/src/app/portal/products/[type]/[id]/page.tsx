...
export default function PortalProductDetailPage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  const typeLabel = productTypeLabels[item.productType] ?? item.productType
  const productName = getProductNameForItem(item)
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link href={`/portal/products/${type}`}><ArrowLeftIcon className="mr-1 size-4" />Back</Link>
        </Button>
        <div>
