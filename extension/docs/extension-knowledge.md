...
- BM Invite: click `featInvite` → `init()` (4 bước, progress) → `main`/`error`. **Multi-email:** `goads-email` là **textarea** (1 email/dòng, hoặc phân tách bằng phẩy/space/`;`). `parseEmails()` validate + dedupe. `handleInvite()` loop từng email → mỗi email 1 row kết quả (Sent/Failed) trong `goads-resultList` + summary banner (success/warn/error). Ctrl/Cmd+Enter để gửi. Count badge `goads-emailCount` live.
